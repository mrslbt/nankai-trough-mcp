...
Every tool is registered in `index.ts` with **annotations + parameter titles**. Use this shape, and don't drop the `.meta({ title })` or the annotation:
...
2. **Every figure cites a primary source + as-of date.** No number ships without a `.go.jp` source and the date verified. If a value can't be confirmed in a primary source, **drop it and point to the source**, never repeating a press figure as fact. (See the retired "149 municipalities" note in `nankai.ts`.)
